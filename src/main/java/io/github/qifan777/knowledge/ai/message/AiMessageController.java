...
import io.github.qifan777.knowledge.ai.personlity.Personlity;
import io.github.qifan777.knowledge.ai.personlity.PersonlityRespository;
...
public class AiMessageController {
    ...
    public void save(@RequestBody AiMessageInput input) {
        AiMessage aiMessage = input.toEntity();

        System.out.println(aiMessage.id());
...
//    }
    ...
    public Flux<ServerSentEvent<String>> chat(@RequestPart String input, @RequestPart(required = false) MultipartFile file) {
        PersonlityRespository personlityRespository = applicationContext.getBean(PersonlityRespository.class);
        List<Personlity> personlities = personlityRespository.findByUserId();


        AiMessageWrapper aiMessageWrapper = objectMapper.readValue(input, AiMessageWrapper.class);
        //说话之前输入分析得到的用户人格特质
        if(!personlities.isEmpty()){
            String personality = "宜人性:"+personlities.get(0).agr()+"开倾性"+personlities.get(0).ext()+"神经质:"+personlities.get(0).neu()+
                    "尽责性:"+personlities.get(0).con()+"外向性:"+personlities.get(0).ope()+"。";
            AiMessageInput aiMessageInput = new AiMessageInput(aiMessageWrapper.getMessage().toEntity());
            aiMessageInput.setTextContent(personality+aiMessageInput.getTextContent());
            aiMessageWrapper.setMessage(aiMessageInput);
        }
